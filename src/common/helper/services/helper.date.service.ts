...
import type { IHelperDateService } from "src/common/helper/interfaces/helper.date-service.interface";
import type {
...
export class HelperDateService implements IHelperDateService {
  ...
  diff(dateOne: Date, dateTwoMoreThanDateOne: Date, options?: IHelperDateOptionsDiff): number {
    ...
    if (options?.format === ENUM_HELPER_DATE_DIFF.SECONDS) {
      return diff.asSeconds();
    }
    if (options?.format === ENUM_HELPER_DATE_DIFF.HOURS) {
      return diff.asHours();
    }
    if (options?.format === ENUM_HELPER_DATE_DIFF.MINUTES) {
